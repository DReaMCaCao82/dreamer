					        			...
					        			<c:when test="${status.index eq 0 }">
					        				<div>
								        		<div id="image1" name="image1" style="margin:0 auto; margin-top: 5%; height: 162px; width: 162px; border: 1px solid gray; background-repeat: no-repeat; background-size: cover;
								        											  background-image: url('/app/images/${thisBBS['OPERATOR']}/${thisBBS['MEETINGNAME']}/BBS/${thisBBS['BBSNO']}/${images['IMG']}');">
					     				 			<div>
						     				 			<label for="cma_file1" disabled="disabled">사진 선택</label>
											 			<input type="file" name="cma_file" id="cma_file1" value="${images['IMG'] }" onchange="getThumbnailPrivew(this,$('#image1'))"/>
											 			<input type="hidden" name="imgNo" value="${images['IMGNO'] }"/>
										 			</div>
									 			</div>
									 			<input type="hidden" style="background-color:#03318c; color:white; border:none; border-radius:5px; height:30px; margin-left:50px;" value="삭제" onclick="remove_div(this)">
									 		</div>
					        			</c:when>
					        			<c:otherwise>
					        				<div>
